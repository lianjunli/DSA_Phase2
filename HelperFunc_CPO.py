...
def capacity_SU(p, SU_index, channel_alloc, power_alloc, channel_gain, B, noise_vec, SNR_gap):
    ...
    for m in range(n_channel):
        if (channel_alloc[n, m] == 1):
            ...
            for j in range(n_su):
                if (j != n):
                    inter_sum = inter_sum + channel_alloc[j, m] * channel_gain[n, j] * power_alloc[j, m]
            capacity_sum = capacity_sum +  B / (10 ** 6) * \
                           np.log2(1 + channel_gain[n, n] / SNR_gap[n] * p[m] / (inter_sum + noise_vec[n]))
    return capacity_sum

def capacity_network(channel_alloc, power_alloc, channel_gain, B, noise_vec, SNR_gap):
    # get objective function
    capacity_sum = 0
    n_channel = power_alloc.shape[1]
    n_su = power_alloc.shape[0]
...
def check_feasibility(h, gamma, noise_mat, cg, power_all, channel_cluster, i_CG):
    power = power_all[channel_cluster]
    ...
    if spec_radius < 1:
        ...
        if np.sum(p_min > np.asarray(power)) == 0:
        ...
        p_min_scaled = np.min(np.asarray(power) / p_min) * p_min
    ...
    return is_feasible, p_min, p_min_scaled
...
def diff_in_throughput(i_UE, i_CG, channel_cluster0, noise_mat, SNR_gap, QoS, cg, p_max, ch_gains, shadow_Fading_Margin):
    ...
    for n in range(num_UE_0):
        ...
        sum_of_ph = np.sum(p_max[channel_cluster0] * ch_gains0[:, n]) - p_max[channel_cluster0[n]] * ch_gains0[n, n]
        sum_rate0 += B * np.log2(1 + ch_gains0[n, n] * p_max[channel_cluster0[n]] / SNR_gap[channel_cluster0[n]] / (sum_of_ph + Noise))
    ...
    gamma = np.multiply(2 ** (QoS[channel_cluster] / B) - 1, SNR_gap[channel_cluster]) * (10 ** (shadow_Fading_Margin / 10))

    is_feasible, p_min, p_min_scaled = check_feasibility(ch_gains, gamma, noise_mat, cg, p_max, channel_cluster, i_CG)
    ...
    for n in range(num_UE):
        ...
        for i in range(len(cg.channel_groups[i_CG])):
            Noise += noise_mat[channel_cluster[n], np.where(np.asarray(cg.channel_IDs) == cg.channel_groups[i_CG][i])[0][0]]
        sum_of_ph = np.sum(p_max[channel_cluster] * ch_gains[:, n]) - p_max[channel_cluster[n]] * ch_gains[n, n]
        sum_rate += B * np.log2(1 + ch_gains[n, n] * p_max[channel_cluster[n]] / SNR_gap[channel_cluster[n]] / (sum_of_ph + Noise))
...
def translate_to_std_output(channel_allocation, channel_groups, cluster_ID, power_allocation, rate, sort_output = False):
    out_cluster_IDs=[]
    out_1st_channel_idx=[]
    out_num_channels=[]
    out_power=[]
    ...
    if sort_output:
        ...
        out_rate = [out_rate[i] for i in idx]
    return out_cluster_IDs, out_1st_channel_idx, out_num_channels, out_power, out_rate
